		...
		if event.Type == linebot.EventTypeMessage {
			...
			case *linebot.TextMessage:
				...
				case strings.EqualFold(message.Text, "2"):
					...
					var user CustData
					for results.Next() {
						err = results.Scan(&user.fullName, &user.mobile, &user.sumPrice, &user.orderStatus)
						if err != nil {
							panic(err.Error())
						}
						if _, err = bot.ReplyMessage(
							event.ReplyToken,
							linebot.NewTextMessage("訂單歷史= "+user.fullName+user.mobile+user.sumPrice+user.orderStatus)).Do(); err != nil {
							log.Println("err:", err)
							return
						}
						return
